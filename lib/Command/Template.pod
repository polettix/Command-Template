...
Both adhere to the same L</Expansion Rules> explained below.
...
L<Command::Template::Instance> (and any other module relying on it).
...
Returns a L<Command::Template::Runner> object with a C<run> method.
...
Returns a L<Command::Template::Instance> object with a C<command> method
